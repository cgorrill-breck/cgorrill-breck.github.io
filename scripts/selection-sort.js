...
function SelectionSortVisualizer() {
    ...
    const [i, setI] = useState(0); // Target boundary for sorted section
    const [j, setJ] = useState(1); // Scanning pointer
    const [extremeIdx, setExtremeIdx] = useState(0); // Index of current min/max

    // States: COMPARING, EVALUATED_NEW, EVALUATED_NO, SWAPPED, DONE
    ...
    const extremeWord = isAscending ? 'minimum' : 'maximum';
    ...
    const handleStep = useCallback(() => {
        ...
        if (stepState === 'COMPARING') {
            const oldExtremeVal = array[extremeIdx];
            const isNewExtreme = isAscending ? array[j] < oldExtremeVal : array[j] > oldExtremeVal;

            if (isNewExtreme) {
                setExtremeIdx(j); // Update immediately for visuals
                setStepState('EVALUATED_NEW');
                setLog(`${array[j]} is ${isAscending ? '<' : '>'} ${oldExtremeVal}. Found a new ${extremeWord}!`);
            } else {
                setStepState('EVALUATED_NO');
                setLog(`${array[j]} is not ${isAscending ? '<' : '>'} ${oldExtremeVal}.`);
            }
        } else if (stepState === 'EVALUATED_NEW' || stepState === 'EVALUATED_NO') {
            if (j >= array.length - 1) {
                // Reached the end of the pass
                if (i !== extremeIdx) {
                    const newArr = [...array];
                    [newArr[i], newArr[extremeIdx]] = [newArr[extremeIdx], newArr[i]];
                    setArray(newArr);
                    setStepState('SWAPPED');
                    setLog(`End of pass. Swapped target (index ${i}) with ${extremeWord} (index ${extremeIdx}).`);
                } else {
                    setStepState('SWAPPED');
                    setLog(`End of pass. The ${extremeWord} is already in the correct position!`);
                }
            } else {
                setJ(j + 1);
                setStepState('COMPARING');
                setLog(`Comparing next element...`);
            }
        } else if (stepState === 'SWAPPED') {
            const newI = i + 1;
            setI(newI);
            ...
            if (newI >= array.length - 1) {
                ...
                setJ(newI + 1);
                setStepState('COMPARING');
                setLog(`Locked in sorted position. Starting pass for the next ${extremeWord}.`);
            }
        }
    }, [array, i, j, extremeIdx, stepState, isAscending, isComplete, extremeWord]);
    ...
    const getBarColor = (index) => {
        ...
        if (index < i) return 'bg-emerald-600'; // Locked in sorted position

        if (stepState === 'SWAPPED') {
            if (index === i || (index === extremeIdx && i !== extremeIdx)) return 'bg-orange-500';
            if (index === extremeIdx && i === extremeIdx) return 'bg-lime-400'; // Correct position
        }

        if (index === extremeIdx) return 'bg-yellow-400'; // Current min/max

        if (index === j) {
            if (stepState === 'COMPARING') return 'bg-blue-400';
            if (stepState === 'EVALUATED_NO') return 'bg-slate-400'; // Dim if check failed
        }

        if (index === i) return 'bg-indigo-200'; // The target slot to fill
        ...
        <div className="min-h-screen bg-slate-50 text-slate-800 p-8 font-sans">
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="text-center space-y-2">
                    <h1 className="text-4xl font-bold text-slate-900">Selection Sort Visualizer</h1>
                    <p className="text-slate-600 text-lg">Watch the algorithm scan for the extreme and swap it into place.</p>
                </div>
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <div className="flex justify-center items-end h-48 gap-4 mb-4">
                        {array.map((val, idx) => (
                            <div key={idx} className="flex flex-col items-center gap-2">
                                ...
                                <div className="h-8 flex flex-col items-center justify-start mt-2">
                                    {!isComplete && (idx === i || idx === j || idx === extremeIdx) && (
                                        <>
                                            ...
                                            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider mt-1 text-center whitespace-nowrap">
                                                {[
                                                    idx === i ? 'i' : null,
                                                    idx === extremeIdx ? (isAscending ? 'min' : 'max') : null,
                                                    idx === j ? 'j' : null
                                                ].filter(Boolean).join(' / ')}
                                            </span>
                                        </>
                ...
                <div className="flex flex-col gap-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <p className="text-sm text-slate-600 mb-4">
                            Watch how the inner loop only exists to <strong>find</strong> the extreme. The swap happens safely in the outer loop.
                        </p>
                        <pre className="bg-slate-800 text-slate-50 p-4 rounded-xl text-sm md:text-base overflow-x-auto shadow-inner">
                            <code>
                                <span className="text-slate-400">// Outer loop locks in the sorted boundary</span>
                                {`\nfor (int i = 0; i < array.length - 1; i++) {\n  `}
                                <span className="text-slate-400">// Assume the first unsorted element is the extreme</span>
                                {`\n  int extremeIdx = i;\n\n  `}
                                <span className="text-slate-400">// Inner loop to find the actual extreme</span>
                                {`\n  for (int j = i + 1; j < array.length; j++) {\n    `}
                                <span className="bg-indigo-600/50 px-2 py-1 rounded inline-block">
                                    {`if (array[j] ${isAscending ? '<' : '>'} array[extremeIdx]) {`}
                                </span>
                                {`\n      extremeIdx = j;\n    }\n  }\n\n  `}
                                <span className="text-slate-400">// Swap the found extreme into its correct sorted position</span>
                                {`\n  int temp = array[i];\n  array[i] = array[extremeIdx];\n  array[extremeIdx] = temp;\n}`}
                            </code>
                        </pre>
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                        <ul className="flex flex-wrap items-center justify-center gap-6">
                            <li className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded bg-indigo-200 shrink-0"></div>
                                <span className="text-sm font-medium text-slate-700">Target (i)</span>
                            </li>
                            <li className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded bg-yellow-400 shrink-0"></div>
                                <span className="text-sm font-medium text-slate-700">Current Ext.</span>
                            </li>
                            <li className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded bg-blue-400 shrink-0"></div>
                                <span className="text-sm font-medium text-slate-700">Scanning (j)</span>
                            </li>
                            <li className="flex items-center gap-2">
                                <div className="w-4 h-4 rounded bg-orange-500 shrink-0"></div>
                                <span className="text-sm font-medium text-slate-700">Swapping</span>
                            </li>
                            <li className="flex items-center gap-2">
...
const root = ReactDOM.createRoot(document.getElementById('selection-sort'));
root.render(<SelectionSortVisualizer />);