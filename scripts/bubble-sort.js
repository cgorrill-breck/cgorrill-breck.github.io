...
function BubbleSortVisualizer() {
    ...
    const reset = useCallback(
        (newArr = initialArray, newDir = isAscending) => {
            setIsPlaying(false);
            setArray([...newArr]);
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                    <div className="h-16 flex items-center justify-center mb-8 bg-slate-100 rounded-lg">
                        <p className="text-xl font-medium text-slate-700">{log}</p>
                    </div>

                    <div className="flex justify-center items-end h-48 gap-4 mb-12">
                        {array.map((val, idx) => (
                            <div key={idx} className="flex flex-col items-center gap-2">
                                <div className={`w-16 rounded-t-md transition-all duration-300 ${getBarColor(idx)} shadow-sm`} style={{ height: `${val * 15}px` }} />
                                ...
                                <div className="h-6 flex items-center justify-center">
                                    {!isComplete && idx < array.length - i && (idx === j || idx === j + 1) && <span className="text-slate-400 font-bold text-xl">↑</span>}
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-6 border-t border-slate-100 pt-8">
                        <div className="flex flex-wrap justify-center items-center gap-4">
                            <button onClick={() => setIsPlaying(!isPlaying)} disabled={isComplete} className={`flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-white transition-colors ${isPlaying ? "bg-amber-500 hover:bg-amber-600" : "bg-indigo-600 hover:bg-indigo-700"} disabled:opacity-50 disabled:cursor-not-allowed`}>
                                {isPlaying ? <Pause size={20} /> : <Play size={20} />}
                                {isPlaying ? "Pause" : "Auto-Play"}
                            </button>

                            <button onClick={handleStep} disabled={isComplete || isPlaying} className="flex items-center gap-2 px-6 py-3 rounded-full font-semibold bg-slate-800 text-white hover:bg-slate-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                <StepForward size={20} /> Next Step
                            </button>

                            <button onClick={() => reset(initialArray)} className="p-3 rounded-full bg-slate-200 text-slate-700 hover:bg-slate-300 transition-colors" title="Restart">
                                <RotateCcw size={20} />
                            </button>

                            <button onClick={randomize} className="p-3 rounded-full bg-slate-200 text-slate-700 hover:bg-slate-300 transition-colors" title="Randomize Array">
                                <Shuffle size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between w-full max-w-md gap-4">
                            <div className="flex items-center gap-2">
                                ...
                                <input type="range" min="200" max="1500" step="100" value={1700 - speed} onChange={(e) => setSpeed(1700 - parseInt(e.target.value, 10))} className="w-24 accent-indigo-600" />
                            </div>

                            <button onClick={toggleDirection} className="flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition-colors">
                                <ArrowDownUp size={18} />
